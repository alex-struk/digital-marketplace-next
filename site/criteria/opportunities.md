...
| R-1.43 | not testable: blocked: the given is an opportunity that has reached the consensus stage or beyond, which the seed does not carry and which the one closable opportunity of each program can only be walked to once. panel_locked_after_consensus names exactly this outcome and has nothing to be read on. The permitted half of the rule — changing the panel while the opportunity is a draft — is exercised by R-1.55. Unblocked by seeding an opportunity at the consensus stage with an evaluation panel already named. | not-testable |
| R-1.44 | — |  |
| R-1.45 | — |  |
| R-1.46 | — |  |
| R-1.47 | — |  |
