...
| R-5.2 | — |  |
| R-5.3 | not testable: The given is an evaluator who has already started an evaluation of one proponent. An evaluation can be started only while the opportunity is in individual question evaluation, which an opportunity reaches only by closing at its proposal deadline, and no page, action or observation brings that about (see R-1.1). The observation the refusal would be read from, duplicate_evaluation_error, is named on both individual-create surfaces, but nothing in the surface can put an evaluator in front of it. | not-testable |
| R-5.4 | — |  |
| R-5.5 | — |  |
| R-5.6 | — |  |
| R-5.7 | — |  |
