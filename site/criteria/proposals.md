...
| R-2.5 | acceptance/proposals/R-2.5.spec.ts | not-testable |
| R-2.6 | — |  |
...
| R-2.28 | acceptance/proposals/R-2.28.spec.ts | not-testable |
| R-2.29 | not testable: blocked: two things are missing. The ranking and the cut need the panel's agreed scores finalised, which consumes the one seeded closed opportunity of that program (see R-2.27). And the criterion describes six proponents of whom five clear every minimum and four go forward; the seed carries three per opportunity and says so itself. The size of the cut therefore cannot be shown even once, and the half that three proponents could show — one below a question's minimum score being left behind — needs the same finalisation. Unblocked by a closed opportunity of its own carrying six proponents. | not-testable |
| R-2.30 | not testable: blocked: the given is two proposals still in contention with a team scenario score being entered against one of them, which is the last stage of the evaluation. Reaching it means walking the whole chain on the one seeded closed Sprint With Us opportunity and leaving nothing behind for the other criteria (see R-2.27), so the price_score observations on the two proposal views have nothing to return and the lowest bid among proposals in contention is never worked out. Unblocked by a closed opportunity of its own, or by one seeded at the code challenge stage with its question scores already recorded. | not-testable |
...
| R-2.34 | acceptance/proposals/R-2.34.spec.ts | not-testable |
| D-proposals-35 | — |  |
