...
| R-6.2 | not testable: blocked: the given is a mail server that cannot be reached, or a message that cannot be composed. Nothing in the surface or `mail` breaks, holds or intercepts delivery — `mail` only searches and clears the catcher. Unblocked by a target variant whose mail settings point at an unreachable server; the action's own success observation, and an empty catcher once delivery is restored (no retry), would then be readable through what already exists. | not-testable |
| R-6.3 | not testable: blocked: the oracle turns the test marking on, but the contract never names the marker, so a marked subject cannot be told from an unmarked one; and the test logo sits in the message body, which the mail fixture does not return. Needs the marker's text named in observables.yaml and a `mail` accessor that reads one message's HTML body by its ID (observables.yaml already names read_one_message and html_body). | not-testable |
| R-6.4 | not testable: blocked: sender and reply-to are headers of one message, and the mail fixture returns only Subject, Snippet, To and ID. Needs a `mail` accessor for one message's From and Reply-To (observables.yaml names `sender: from` and read_one_message), and the configured sender named in the contract so that 'the same for every kind of message' can be compared against it. | not-testable |
| R-6.5 | not testable: blocked: comparing the plain-text form with the formatted one needs both bodies of one message. The mail fixture exposes neither; its snippet is a summary the catcher makes, not evidence that a plain-text part was sent. Needs a `mail` accessor returning one message's HTML and Text (observables.yaml names html_body and plain_text_body). | not-testable |
...
| R-6.26 | — | fail |
| R-6.27 | acceptance/notifications/R-6.27.spec.ts | not-testable |
