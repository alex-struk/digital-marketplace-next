...
| R-7.17 | not testable: blocked: the criterion's two distinctive clauses each need something the surface does not offer. That embedded markup is never executed cannot be read, because content-view hands back the rendered body as a single piece of text: a body whose markup was run and a body whose markup was taken out come back alike, so no assertion on page_body can fail against a service that executes what a body embeds. That the same body renders identically on its own address and wherever another screen embeds it has no second rendering to compare — the screens that embed a body, the two program qualification terms screens and the two evaluation instructions screens, embed pages the service creates for itself, which the seed does not name and which no surface addresses by handle, so a body a test can author cannot be got into an embedded rendering. The remaining clause, that the body reaches the reader as formatted text, is already asserted by R-7.1 against the same page and the same observation. Unblocked by an observation on content-view reporting whether markup embedded in a body took effect in the rendering rather than reaching the reader as text, and by a seed handle for one of the pages another screen embeds together with an observation of that embedded rendering. | pass |
