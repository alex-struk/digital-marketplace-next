...
| id | test | old |
| --- | --- | --- |
| R-7.1 | acceptance/content/R-7.1.spec.ts | pass |
...
| R-7.11 | — | not-testable |
| R-7.12 | not testable: blocked: the given is a fresh installation carrying the pages the service needs, and no such page can be named. The seed defines one content record, content.ordinary_page, which it records as the only page an administrator made; the pages the service creates for itself arrive with the installation's own migrations and the seed names none of them, so content-view.open and content-edit.open, which both take an address, have no address to be given that comes from the seed rather than from a guess. The count the criterion makes is out of reach for a second reason: content-list reads page_title, page_public_address and page_is_fixed but returns no count of the pages it lists, and the seed adds an ordinary page of its own to whatever the installation carries. Unblocked by the seed naming the pages the service needs — a content group of handles carrying each one's address, so a test can open them by handle instead of by guess — and by a content-list observation of how many pages are listed. | fail (ruled: test-wrong) |
...
| R-7.17 | not testable: blocked: the criterion's two distinctive clauses each need something that is missing, and for the first of them what is missing is the body, not the observation. That embedded markup is never executed can be read off content-view's page_body once there is a body with markup in it: the rendered text of such a body carries the markup as a string when it reaches the reader as text, and does not carry it when it is executed or taken out, and a rendering that takes it out fails the criterion's formatted-text clause as well. The acceptance seed holds no such body — its only page, content.ordinaryPage, carries 'The third and current version of this page, with a **formatted** word in it.' — so the clause has nothing to be read against. That the same body renders identically on its own address and wherever another screen embeds it has no second rendering to compare: the screens that embed a body, the two program qualification terms screens and the two evaluation instructions screens, embed pages the service creates for itself, which the seed does not name and which no surface addresses by handle, so a body a test can author cannot be got into an embedded rendering. The remaining clause, that the body reaches the reader as formatted text, is already asserted by R-7.1 against the same page and the same observation. Unblocked, for the first clause, by a seeded page body embedding markup under a handle of its own, against which page_body can be read; and, for the second, by a seed handle for one of the pages another screen embeds together with an observation of that embedded rendering. | pass |
...
| R-7.25 | not testable: blocked: the given is an administrator on the managing screen of a page the service itself depends on, and no such page can be reached. The seed names one content record and records it as an ordinary page an administrator made, not one the service depends on; the pages the service depends on are created by the installation's own migrations and carry no seed handle, so content-edit.open, which takes an address, has no address for one that comes from the seed rather than from a guess. Every observation the criterion needs is already on content-edit — fixed_page_warning, slug_locked_for_fixed_page and delete_withheld_for_fixed_page — and none of them can be read on a page the criterion is about until one can be opened. Unblocked by the seed naming a page the service depends on, so its address comes from a handle. A separate half stays out of reach even then: "a request to rename or remove it made another way is refused" needs a way to send the service a request other than through its own screens, and the surface has none. | fail (ruled: test-wrong) |
| D-content-26 | — |  |
| R-7.26 | acceptance/content/R-7.26.spec.ts | unbound |
| D-content-27 | — |  |
| R-7.27 | not testable: blocked: both halves of the given are out of reach. The first is a page the service created for itself and nobody has edited: the seed names no such page — its one content record is an ordinary page an administrator made — so content-edit.open has no address for one that comes from a handle rather than from a guess, and content-edit.published_by and updated_by cannot be read where the service is the name they should carry. The second is a page one administrator created and another later changed: the seed carries users.administrator_two and the contract carries persona.administrator_other, but that persona's sign_in marks its session route unavailable — the route mints a session for one fixed administrator and takes no parameter, so no second administrator is reachable through it — and the change the given describes cannot be made by anybody else. Unblocked by the seed naming a page the service created for itself, and by a sign-in that reaches persona.administrator_other on the target under test. | fail (ruled: test-wrong) |
...
### D-content-26 · v1 · inferred · obsolete
...
### D-content-27 · v1 · inferred · obsolete
