...
import { INVALID_PROFILE_ERRORS, PageShell, ProfileScreen, SAMPLE_PUBLIC_SERVANT } from "./users.shared";
...
// Saved with the name cleared and a malformed email address (R-4.27): nothing is saved, the
// summary lists both problems, each field reports its own, and the job title is not an error.
function UserProfileInvalid() {
  return (
    <PageShell>
      <ProfileScreen
        ids={ids}
        ...
        viewer="own"
        mode="edit"
        errors={INVALID_PROFILE_ERRORS}
      />
    </PageShell>
  );
}

const meta: Meta<typeof UserProfileInvalid> = {
  title: "users/user-profile/invalid",
  component: UserProfileInvalid,
  parameters: { layout: "fullscreen", page: "user-profile", route: "/users/:userId", state: "invalid" },
};

export default meta;

export const Invalid: StoryObj<typeof UserProfileInvalid> = {};
