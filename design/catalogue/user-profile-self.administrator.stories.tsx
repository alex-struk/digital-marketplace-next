...
import { PageShell, ProfileScreen, SAMPLE_ADMINISTRATOR } from "./users.shared";
...
// An administrator's own profile: status shown (R-4.34), no administrator box (R-4.12) and no
// deactivation control (R-4.31).
function UserProfileSelfAdministrator() {
  return (
    <PageShell>
      <ProfileScreen ids={ids} person={SAMPLE_ADMINISTRATOR} viewer="own" addressedAsMe showStatus />
    </PageShell>
  );
}

const meta: Meta<typeof UserProfileSelfAdministrator> = {
  title: "users/user-profile-self/administrator",
  component: UserProfileSelfAdministrator,
  parameters: { layout: "fullscreen", page: "user-profile-self", route: "/users/me", state: "administrator" },
};

export default meta;

export const Administrator: StoryObj<typeof UserProfileSelfAdministrator> = {};
