...
Notifications are the service's only way of reaching a person who is not looking at it. Every
notice it sends is an email message composed by the service itself, sent to one person or
broadcast to many, and triggered by something that happened elsewhere in the service — an
opportunity published, a proposal awarded, a team request made, terms changed. This domain is not
about which event notifies whom, which each of the other domains records for its own events; it is
about the delivery machinery every one of those notices passes through, the single preference a
person can set over them, and the two service-wide broadcasts that belong to nothing else.

Two sources describe this machinery outside the code. The service's own README documents the
settings that suppress notifications entirely, mark them as coming from a test environment, cap
the size of a broadcast and name the sender; all four agree with the code. It also describes an
internal page where an administrator can preview every message the service sends, and that one
does not hold: the page exists and is administrator-only as described, but twelve of the
messages are absent from it. The published interface description covers the setting of the
notification preference and both watch actions, but says nothing at all about the broadcast that
announces changed terms and conditions, so that route is undocumented. Nothing outside the code
describes the content of any individual message.

Where a criterion below touches an event another domain already records, it states only what this
domain adds — the shape of the message, or the route it takes to a person — and leaves the
question of who is notified where it already sits.

### D-notifications-1 · v1 · confirmed · recovered
...
### D-notifications-2 · v1 · inferred · recovered
...
### D-notifications-3 · v1 · confirmed · recovered
...
### D-notifications-4 · v1 · confirmed · recovered
Every message the service sends comes from a single named sending address that does not accept replies.
...
### D-notifications-5 · v1 · inferred · recovered
...
### D-notifications-6 · v1 · inferred · recovered
Every message the service sends ends with a way to stop receiving notifications.
...
- note: the offer appears on every message the service sends, including the ones the notification preference does not govern — see D-notifications-10.

### D-notifications-7 · v1 · inferred · recovered
...
### D-notifications-8 · v1 · confirmed · recovered
...
### D-notifications-9 · v1 · inferred · recovered
...
### D-notifications-10 · v1 · inferred · recovered
...
### D-notifications-14 · v1 · inferred · recovered
...
### D-notifications-16 · v1 · inferred · recovered
...
- note: the withdrawal of acceptances is completed before the response, so the part of the action a vendor will notice on their next visit is reliable; only the messages are not. Delivery failures are invisible for the reason given in D-notifications-2, so a run that reaches nobody looks exactly like one that reaches everybody.

### D-notifications-18 · v1 · confirmed · recovered
...
### D-notifications-19 · v1 · open · recovered
