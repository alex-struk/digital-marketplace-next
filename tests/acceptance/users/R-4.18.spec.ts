...
// is not the requester's own is not asserted. Every profile change the surface can make goes
// through the editing control, and the claim is that an administrator is not offered it, so
// nothing is left to press and nothing in the surface sends the change another way.
//
// The administrator's view is compared with the owner's view of the same address, so the
// absence of the editing control is the absence of something that screen does offer its
// owner rather than a word the screen never uses.
