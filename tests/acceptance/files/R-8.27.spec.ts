...
// The criterion's own example: "scan0001.pdf" renamed to "Statement of work", with the ending
// left off, and stored as "Statement of work.pdf". The rename happens while the attachment is
// only listed on the form, which is the "before it is uploaded" the criterion turns on; that
// an attachment already stored is shown read-only is read back after the save as the other
...
test("an attachment can be given a different display name before it is uploaded, and the ending of the original file is put back on if the person leaves it off", async ({
  ...
  await surface.fileAttachmentControl.addAttachment({ file: "scan0001.pdf" });
  ...
  await surface.opportunityCwuCreate.saveDraft({ title: "R-8.27 draft with a renamed attachment" });
